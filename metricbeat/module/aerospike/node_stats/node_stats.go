...
func (m *MetricSet) Fetch(reporter mb.ReporterV2) error {
	...
	for _, node := range m.client.GetNodes() {
		info, err := as.RequestNodeInfo(node, "node")
		if err != nil {
			m.Logger().Error("Failed to retrieve node stats from node %s", node.GetName())
			continue
		}
	...
	//}
		...
		for key, value := range info {
			// Apply necessary schema transformations here if any
			data[key] = value
		}
		data["node"] = mapstr.M{
			"host": node.GetHost().String(),
